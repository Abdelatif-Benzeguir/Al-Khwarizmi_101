...
// متغيرات نظام التحميل التقادمي (Progressive Loading)
let resourcesPerPage = 6;
let currentPage = 1;
...
/* ============================================================
   ...
   ============================================================ */
/* ============================================================
   استخراج معرف فيديو يوتيوب من الرابط (YouTube ID Extractor)
   ============================================================ */
...
/* ============================================================
   حساب الوقت النسبي للعرض (Relative Time)
   ============================================================ */
...
/* ============================================================
   تحويل الدقائق إلى صيغة عرض المدة (Duration Formatter)
   ============================================================ */
...
/* ============================================================
   عرض الموارد التعليمية بنظام التحميل التقادمي (Progressive Render)
   ============================================================ */
function renderResources() {
  const grid = document.getElementById('resourcesGrid');
  const loadMoreContainer = document.getElementById('loadMoreContainer');

  // حالة التحميل الأول: عرض هياكل تحميل وهمية
  if (resourcesFirstLoad) {
    const skeletonCards = Array(6).fill('').map((_, i) => {
      // تنويع بين هيكل كرت عادي وهيكل كرت فيديو
      if (i % 3 === 0) {
        return `
        <div class="video-card">
          ...
          <div style="padding: 0.85rem 1rem;">
            <div class="skeleton-meta-row">
              ...
              <div style="flex:1;">
                <div class="skeleton" style="height: 0.9rem; width: 85%; margin-bottom: 0.4rem;"></div>
                <div class="skeleton" style="height: 0.7rem; width: 55%;"></div>
              </div>
            </div>
          </div>
        </div>`;
  ...
  const visibleCount = currentPage * resourcesPerPage;
  const visibleItems = filtered.slice(0, visibleCount);
  const hasMore = visibleCount < filtered.length;

  grid.innerHTML = visibleItems.map((r, index) => {
    ...
    const animDelay = index >= (currentPage - 1) * resourcesPerPage ? `style="animation-delay: ${(index % resourcesPerPage) * 0.08}s"` : '';
    const fadeClass = index >= (currentPage - 1) * resourcesPerPage ? 'card-fade-in' : '';

    // === كرت فيديو بتصميم يوتيوب ===
    if (r.type === '\u0641\u064a\u062f\u064a\u0648') {
      const videoId = extractYouTubeId(r.link);
      const thumbnailUrl = videoId
        ? `https://img.youtube.com/vi/${videoId}/mqdefault.jpg`
        : 'https://placehold.co/480x270/1e293b/64748b?text=\u0641\u064a\u062f\u064a\u0648';
      const durationDisplay = formatDuration(r.duration);
      const relTime = getRelativeTime(r.createdAt);

      return `
      <div class="video-card ${fadeClass}" ${animDelay} onclick="openVideoModal('${videoId || ''}', '${escapeHTML(r.title).replace(/'/g, "\\'")}'${!videoId ? `, '${sanitizeURL(r.link)}'` : ''})">
        ${isNew ? '<span class="badge-new">\u062c\u062f\u064a\u062f</span>' : ''}
        <div class="video-thumbnail-wrap">
          <img src="${thumbnailUrl}" alt="${escapeHTML(r.title)}" class="video-thumbnail" loading="lazy">
          <div class="video-overlay">
            <div class="video-play-icon">
              <i class="fa-solid fa-play" style="margin-right: -2px;"></i>
            </div>
          </div>
          ${durationDisplay ? `<span class="video-duration-badge">${durationDisplay}</span>` : ''}
        </div>
        <div class="video-info-section">
          <img src="https://i.ibb.co/wZ5MX8R4/4897896.jpg" alt="" class="video-channel-avatar" loading="lazy">
          <div class="video-text-block">
            <h4 class="video-card-title">${escapeHTML(r.title)}</h4>
            <div class="video-meta-line">
              <span class="meta-channel">\u0646\u0627\u062f\u064a \u0627\u0644\u062e\u0648\u0627\u0631\u0632\u0645\u064a</span>
              <i class="fa-solid fa-circle meta-dot"></i>
              <span>${relTime}</span>
            </div>
          </div>
        </div>
        ${currentUserData && currentUserData.role === 'teacher' ? `
          <div class="video-admin-actions" onclick="event.stopPropagation()">
    ...
    return `
    <div class="resource-card ${fadeClass}" ${animDelay}>
      ${isNew ? '<span class="badge-new">\u062c\u062f\u064a\u062f</span>' : ''}
  ...
  // تفعيل مراقب التمرير اللانهائي
  setupInfiniteScroll();
}

const debouncedRenderResources = debounce(() => { currentPage = 1; renderResources(); }, 300);
...
function setFilter(f) {
  currentFilter = f;
  currentPage = 1; // إعادة تعيين الصفحة عند تغيير الفلتر
...
/* ============================================================
   نافذة مشغل الفيديو المسرحي (Video Theater Modal)
   ============================================================ */
function openVideoModal(videoId, title, fallbackUrl) {
  ...
  // إذا كان رابط يوتيوب صالح، استخدم iframe المضمّن
  if (videoId) {
    iframe.src = `https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&rel=0&modestbranding=1`;
  } else if (fallbackUrl) {
    // إذا لم يكن يوتيوب، افتح الرابط في تبويب جديد
  ...
  titleEl.textContent = title || 'فيديو تعليمي';

  // إظهار النافذة مع حركة
...
function closeVideoModal(event, forceClose) {
  const overlay = document.getElementById('videoModalOverlay');
  const content = overlay?.querySelector('.video-modal-content');

  // إغلاق فقط عند النقر على الخلفية أو زر الإغلاق
  if (!forceClose && event && content && content.contains(event.target)) {
    return;
  }
  ...
  if (overlay) {
    ...
    // تأخير إيقاف الفيديو حتى انتهاء حركة الإغلاق
    setTimeout(() => {
      const iframe = document.getElementById('videoModalIframe');
...
/* ============================================================
   نظام التمرير اللانهائي (Infinite Scroll via Intersection Observer)
   ============================================================ */
function setupInfiniteScroll() {
  ...
  // إنشاء مراقب جديد إذا كان زر التحميل ظاهراً
...
/* ============================================================
   تحميل المزيد من الموارد (Load More Resources Handler)
   ============================================================ */
function loadMoreResources() {
  ...
  if (loadMoreBtn) {
    ...
    loadMoreBtn.querySelector('i').className = 'fa-solid fa-spinner fa-spin';
  }

  // محاكاة تأخير قصير لتجربة مستخدم أنيقة مع الهيكل التحميلي
  setTimeout(() => {
    currentPage++;
    renderResources();
    isLoadingMore = false;
    ...
    if (loadMoreBtn) {
      ...
      loadMoreBtn.querySelector('span').textContent = 'تحميل المزيد من الموارد';
      loadMoreBtn.querySelector('i').className = 'fa-solid fa-angles-down';
    }
  }, 400);
}
...
/* ============================================================
   مستمعات الأحداث العامة (Global Event Listeners)
   ============================================================ */
...
// مفتاح Escape لإغلاق نوافذ الفيديو
